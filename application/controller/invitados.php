...
class Invitados extends Controller
{
    ...
    {
        //$count = $this->model->traerTodosInvitados(true);
        $invitados = $this->model->getInvitados();
    ...
    {
        if (isset($_POST["submit_agregar_invitado"])) {
            $this->model->agregarInvitado($_POST["codigo"], $_POST["nombresyapellidos"],  $_POST["barras"]);
        }
        header('location: ' . URL . 'invitados/index?m=1');
    }
    ...
    { 
        if (isset($codigo)) {
            $this->model->borrarInvitado($codigo);
        }
        header('location: ' . URL . 'invitados/index?m=2');
    }
     ...
     */
    public function modificar($song_id)
    {
